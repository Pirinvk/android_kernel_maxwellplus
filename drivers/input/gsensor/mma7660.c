...
#define rk28printk(x...)
#endif
#if defined(CONFIG_BQ_C1014)
static int gsensor_info[9]={-1,0,0,0,0,1,0,-1,0};
#elif defined(CONFIG_BQ_C7018)
static int gsensor_info[9]={0,-1,0,0,0,1,1,0,0};
#elif defined(CONFIG_BQ_C8002)
static int gsensor_info[9]={1,0,0,0,0,1,0,-1,0};
#else 
static int gsensor_info[9]={0,1,0,0,0,1,-1,0,0};
#endif
...
static int mma7660_get_data(struct i2c_client *client)
{
	...
	axisback.x =  mma7660_convert_to_int(buffer[MMA7660_REG_X_OUT]);
	axisback.y =  mma7660_convert_to_int(buffer[MMA7660_REG_Y_OUT]);

		...
		case Sensor_Flash:
		{
			...
			if (sensor->sensor_io_request && sensor->sensor_io_request->sensor_ioctrl) {
                ...
                if(on==1){
            		...
            		hrtimer_start(&(flash_off_timer.timer),ktime_set(0, 800*1000*1200),HRTIMER_MODE_REL);
                    }
			}
			break;
		}
...
static int sensor_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	flash_on_off=0;
		...
		#if CONFIG_SENSOR_Focus	
		printk("honghaishen_test s_stream 0000000000000000000000001 \n");
              // flush_workqueue(sensor->sensor_wq);
		printk("honghaishen_test s_stream 0000000000000000000000002 \n");
		#endif
	}
	return 0;
}

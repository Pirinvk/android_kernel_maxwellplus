		...
		case Sensor_Flash:
		{
			...
			if (sensor->sensor_io_request && sensor->sensor_io_request->sensor_ioctrl) {
                ...
                if(on==Flash_On){
            		...
            		hrtimer_start(&(flash_off_timer.timer),ktime_set(0, 800*1000*1300),HRTIMER_MODE_REL);
                    }
			}
            break;
		}
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
    ...
    if (value  == SENSOR_ID) {
		...
		printk("honghaishen_test ok222   22222222222222222222\n");
    } else {
        printk("honghaishen_test no 22 222222222222222222 \n");
        ret = -ENODEV;
        goto sensor_INIT_ERR;
    }
...
{
    printk("honghaishen_test set s_fmt");
    struct i2c_client *client = v4l2_get_subdevdata(sd);
    struct sensor *sensor = to_sensor(client);
    const struct sensor_datafmt *fmt;
	const struct v4l2_queryctrl *qctrl;
...
{    
	...
	{       /* ddl@rock-chips.com: torch */
        ...
        {
        	 sensor_ioctrl(icd, Sensor_Flash, Flash_Torch_Off);   /* Flash On */
          //  sensor_ioctrl(icd, Sensor_Flash, Flash_On);
          flash_on_off=1;
        }
	else if(value ==0)
			       ...
			       struct i2c_client *client)
{
    ...
    if (value == SENSOR_ID) {
        sensor->model = SENSOR_V4L2_IDENT;
		printk("honghaishen_test ok ooooooooooooooo \n");
    } else {
        printk("honghaishen_test no nnnnnnnnnnnnnn \n");
        ret = -ENODEV;
        goto sensor_video_probe_err;
    }
...
struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if (enable)
	{
	printk("honghaishen_test flash is on \n");
	if(flash_on_off==1)
		{
	sensor_ioctrl(icd, Sensor_Flash, Flash_On);   // honghaishen_test hhs_0111
	flash_on_off=0;
		}
	}
}
